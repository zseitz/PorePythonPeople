...
## EventClassifierGUI
*Description:* Creates a GUI using tkinter. This GUI-based application utilizes the search results generated by subcomponent 4 to assist with classifying events in the data. It uses the same directory location specified by the user in subcomponent 4 (loaded from the config file), unless the directory is not found or the user changes it. The GUI displays available search queries from the `tests` directory. When a query is selected, it loads the list of selected files from the search log. The user can then select a file to view its data. Each selected file is a folder containing MAT files; clicking on a file loads and plots `reduced.vdata` vs `reduced.pt` (point index) using matplotlib.

*Key Features:*
- Directory persistence: Loads the database directory from the same config file as subcomponent 4.
- Query selection: Dropdown to choose from saved search queries in the `tests` folder.
