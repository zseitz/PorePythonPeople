...
- **Key behavior**:
  ...
  - Deterministic `sequence_designer_gui.py` fallback scaffolding now includes mlapp-style operator controls (feeding orientation, read direction, panel instructions/notations, and direction-aware plotting annotations) so assistant-driven runs remain usable when implement-stage model actions are unavailable.
